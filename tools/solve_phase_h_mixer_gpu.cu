...
static void candidate_from_counter_host(const uint8_t old_base[OLD_LEN],
                                        const uint8_t workspace_base[USED_FIXED],
                                        unsigned old_hidden,
                                        unsigned ws_hidden,
                                        uint64_t counter,
                                        uint8_t old_state[OLD_LEN],
                                        uint8_t workspace[USED_FIXED]) {
    memcpy(old_state, old_base, OLD_LEN);
    memcpy(workspace, workspace_base, USED_FIXED);
    ...
    if (ws_hidden) {
        ws_value = counter >> old_hidden;
    }

    set_low_bits(old_state, OLD_LEN, old_value, old_hidden);
    set_low_bits(workspace, USED_FIXED, ws_value, ws_hidden);
}

static void fast_first_host(const uint8_t old_base[OLD_LEN],
                            const uint8_t workspace_base[USED_FIXED],
                            const uint32_t prefix_state[5],
                            const uint8_t digest_a[20],
                            unsigned old_hidden,
                            unsigned ws_hidden,
                            uint64_t counter,
                            uint8_t out[20]) {
    ...
    uint8_t msg[CMSG_LEN];
    size_t pos = 0;
    memcpy(msg + pos, old_state + 40u, 20u); pos += 20u;
    memcpy(msg + pos, workspace + 2u * QLEN, QLEN); pos += QLEN;
    memcpy(msg + pos, old_state + 60u, 20u); pos += 20u;
    memcpy(msg + pos, workspace + 3u * QLEN, QLEN); pos += QLEN;

    if (pos != CMSG_LEN) {
        fprintf(stderr, "internal host fast-path C message size error\n");
        exit(2);
    }

    uint32_t state[5];
    memcpy(state, prefix_state, 5u * sizeof(uint32_t));

    sha1_compress_host(state, msg + 128u, 1);
    sha1_compress_host(state, msg + 192u, 1);
    sha1_compress_host(state, msg + 256u, 1);

    uint8_t final_block[64];
    memset(final_block, 0, sizeof(final_block));
    memcpy(final_block, msg + 320u, 20u);
    final_block[20] = 0x80u;

    /* 340 bytes = 2720 bits = 0x00000aa0, little-endian length bytes. */
    final_block[60] = 0xA0u;
    final_block[61] = 0x0Au;
    final_block[62] = 0x00u;
    final_block[63] = 0x00u;
    sha1_compress_host(state, final_block, 0);

    uint8_t digest_c[20];
    for (unsigned i = 0; i < 5; ++i) {
        wr_le32(digest_c + 4u * i, state[i]);
    }

    uint8_t pair[40];
    memcpy(pair, digest_a, 20u);
    memcpy(pair + 20u, digest_c, 20u);
    ksec_hash_host(pair, 40u, out);
}

static int host_fast_path_crosscheck(const uint8_t old_base[OLD_LEN],
                                     const uint8_t workspace_base[USED_FIXED],
                                     const uint32_t prefix_state[5],
                                     const uint8_t digest_a[20],
                                     unsigned old_hidden,
                                     unsigned ws_hidden,
                                     uint64_t space) {
    const uint64_t samples[2] = {0u, space - 1u};

    for (unsigned s = 0; s < 2; ++s) {
        uint8_t old_state[OLD_LEN];
        uint8_t workspace[USED_FIXED];
        uint8_t exact[TARGET_LEN];
        uint8_t fast_first[20];
        ...
        replay_mixer_host(workspace, old_state, exact);
        ...
        if (memcmp(fast_first, exact, 20u) != 0) {
            fprintf(
                stderr,
                "HOST_FAST_PATH_CROSSCHECK=FAIL sample=%u counter=%" PRIu64 "\n",
                s,
                samples[s]
            );
            return 0;
...
int main(int argc, char **argv) {
    ...
    /*
     * Real host cross-check of the optimized rejection path against the exact
     * mixer model. Counter 0 and counter 2^K-1 are both checked; the latter
     * exercises every hidden low bit.
     */
    if (!host_fast_path_crosscheck(
            old_base,
            workspace_base,
            prefix_state,
            digest_a,
    ...
    printf("HOST_FAST_PATH_CROSSCHECK_SAMPLES=2\n");
    printf("HOST_FAST_PATH_CROSSCHECK=PASS\n");
    printf("START=%" PRIu64 "\n", start);
